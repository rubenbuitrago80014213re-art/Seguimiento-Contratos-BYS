# app_seguimiento_contratos_mejorado_v6.py
# Streamlit app: seguimiento de contratos - CRUD (SQLite) + alertas (semáforo) + tablero
# Requisitos:
...
# CSS para mejorar accesibilidad/visualidad
st.markdown(
    """
    <style>
    /* Aumentar tamaño de fuente general */
    html, body, [class*="css"]  { font-size: 16px !important; }
    /* Inputs más altos y con mejor contraste */
    ...
    /* Dataframe: envolver texto */
    .stDataFrame [data-testid="stVerticalBlock"] { overflow: visible !important; }
    </style>
    """,
...
# ---- Sidebar con botones (Exportar Excel queda de último) ----
...
MENU = [
    ...
    ("✏️ Editar registro", "Editar registro"),
    ("🗑️ Eliminar registro", "Eliminar registro"),
    ("📊 Tablero de Control", "Tablero de Control"),
    ("🚨 Alertas de Vencimiento", "Alertas de Vencimiento"),
    ("📤 Exportar Excel", "Exportar Excel")  # último
]

if 'current_page' not in st.session_state:
    st.session_state.current_page = "Ver Contratos"

for icon_label, page_value in MENU:
...
if st.session_state.current_page == "Tablero de Control":
    ...
    if not df_all.empty:
        ...
        selected_contract = st.selectbox(
            ...
            help="Puedes ver un análisis global o centrado en un código específico."
        )
    ...
    if not df_display.empty:
        ...
        # Métricas adicionales si hay contrato seleccionado
        if selected_contract != 'Todos los Contratos':
            contrato_iniciado = "Sí" if df_display[df_display["Estado Actual del Proceso"] == "Iniciado"].shape[0] > 0 else "No"
            valor_estimado = pd.to_numeric(df_display["Valor estimado en la vigencia actual"], errors='coerce').sum()
            valor_contratado = pd.to_numeric(df_display["Valor contratado"], errors='coerce').sum()
...
# --- Ver contratos (con filtros accesibles) ---
    ...
    with st.expander("Filtros avanzados 🔎"):
        filter_cols = st.multiselect(
            ...
            options=COLUMNS,
            help="Activa los filtros que necesites para acotar los resultados."
        )
        if filter_cols:
            for col in filter_cols:
                if "Fecha" in col:
                    ...
                    try:
                        ...
                        if pd.notna(min_val) and pd.notna(max_val):
                            min_input, max_input = st.slider(
                                ...
                                help="Arrastra para definir el rango de valores."
                            )
                    ...
                    selected_options = st.multiselect(
                        ...
                        help="Puedes elegir múltiples opciones."
                    )
                    if selected_options:
                    ...
                    search_term = st.text_input(
                        ...
                        help="Búsqueda contiene (no sensible a mayúsculas)."
                    )
                    if search_term:
    ...
    if filtered_df.empty:
        ...
        st.dataframe(
            df_display[cols_show].sort_values(by='Código Interno / Proceso', ascending=True),
            use_container_width=True
        )

# --- Agregar registro ---
elif st.session_state.current_page == "Agregar registro":
    st.header("🆕 Agregar nuevo registro de Bienes y Servicios")
    st.markdown("---")
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        with tab_info_basica:
            cols1 = st.columns(2)
            inputs["Código Interno / Proceso"] = cols1[0].text_input("Código Interno / Proceso", placeholder="Ej: 2025-001", help="Código interno único del proceso.")
            inputs["Nombre del Proceso / Objeto del Contrato"] = cols1[1].text_input("Nombre del Proceso / Objeto del Contrato", placeholder="Objeto general del contrato")
            cols2 = st.columns(3)
            inputs["Estado Actual del Proceso"] = cols2[0].selectbox("Estado Actual del Proceso", options=[''] + ESTADO_PROCESO_OPTS, help="Estado en el ciclo contractual.")
            ...
            inputs["Proveedor / Contratista"] = st.text_input("Proveedor / Contratista", placeholder="Nombre o NIT")
            inputs["Supervisor"] = st.text_input("Supervisor", placeholder="Nombre del supervisor")
            inputs["Supervisor (Apoyo)"] = st.text_input("Supervisor (Apoyo)", placeholder="(Opcional)")
        ...
        with tab_valores:
            cols6 = st.columns(3)
            inputs["Valor estimado en la vigencia actual"] = cols6[0].number_input("Valor estimado en la vigencia actual ($)", min_value=0, step=1, help="Solo números.")
        ...
        with tab_otros_detalles:
            inputs["Número del contrato"] = st.text_input("Número del contrato", placeholder="Número (solo dígitos si es posible)")
            ...
            inputs["Enlace SharePoint"] = st.text_input("Enlace SharePoint", placeholder="URL")
            inputs["Seguimiento periódico"] = st.text_input("Seguimiento periódico")
            inputs["Alerta Enviada"] = st.text_input("Alerta Enviada", placeholder="Sí / No")
...
# --- Editar registro ---
    ...
    st.header("✏️ Editar registro existente")
    st.markdown("---")
    if df_all.empty:
...
# --- Eliminar registro ---
    ...
    st.header("🗑️ Eliminar registro existente")
    st.markdown("---")
    if df_all.empty:
...
# --- Exportar Excel (último en el menú) ---
elif st.session_state.current_page == "Exportar Excel":
    st.header("📤 Exportar base a Excel")
    st.markdown("---")
    if df_all.empty:
        ...
        # Convertir a numérico cuando aplique
